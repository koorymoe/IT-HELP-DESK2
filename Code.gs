// ═══════════════════════════════════════════════════════════════
//  IT HELP DESK — Code.gs الكامل v10.2
// ═══════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════
//  نظام الاستلام من الإيميل مباشرة
// ══════════════════════════════════════════════════════
...
function doGet(e) {
  ...
  // إذا في action بالـ URL — معالجة الإيميل
...
function handleEmailClaim(ticketId, userId, token) {
  var expectedToken = generateEmailToken(ticketId, userId);
  var html = '';
  ...
  try {
    ...
    if (!empData) {
      html = buildResultPage('❌ موظف غير موجود', 'رقم البصمة غير موجود في النظام.', '#ef4444');
      return HtmlService.createHtmlOutput(html);
    }

    if (ROLES_IT.indexOf(empData.role) < 0) {
      html = buildResultPage('❌ غير مصرح', 'ليس لديك صلاحية لاستلام البلاغات.', '#ef4444');
      return HtmlService.createHtmlOutput(html);
    }

    // تحقق إذا البلاغ مستلم مسبقاً
      ...
      if (String(allData[i][COL_TICKETS.id]).trim() === ticketId) {
        ...
        if (currentAssigned && currentStatus !== 'جديدة') {
          ...
          html = buildResultPage(
            '⚠️ تم الاستلام مسبقاً',
            'هذا البلاغ تم استلامه مسبقاً من قبل: <strong>' + currentAssigned + '</strong><br>الحالة الحالية: ' + currentStatus,
            '#f59e0b'
          );
          return HtmlService.createHtmlOutput(html);
        }
        ...
        // إشعار لكل فريق IT بأن البلاغ تم استلامه
        notifyTeamAboutClaim(ticketId, empData, allData[i]);

        html = buildResultPage(
          '✅ تم الاستلام بنجاح',
          'تم تسجيل استلامك للبلاغ <strong>' + ticketId + '</strong><br>سيصل إشعار لباقي الفريق بأنك استلمت هذا البلاغ.',
          '#059669',
          ticketId
        );
        return HtmlService.createHtmlOutput(html);
    ...
    html = buildResultPage('❌ خطأ', e.message, '#ef4444');
  }
...
function handleEmailSolve(ticketId, userId, token) {
  ...
  try {
    ...
    var empData = getEmployeeByEmpId(userId);
    if (!empData || ROLES_IT.indexOf(empData.role) < 0) {
      html = buildResultPage('❌ غير مصرح', 'ليس لديك صلاحية لإغلاق البلاغات.', '#ef4444');
      return HtmlService.createHtmlOutput(html);
    }
    ...
    for (var i = 0; i < allData.length; i++) {
      if (String(allData[i][COL_TICKETS.id]).trim() === ticketId) {
        var currentStatus = String(allData[i][COL_TICKETS.status] || '').trim();
        if (currentStatus === 'تم حل البلاغ') {
          html = buildResultPage('✅ البلاغ محلول مسبقاً', 'هذا البلاغ تم حله مسبقاً.', '#059669');
          return HtmlService.createHtmlOutput(html);
        }
        ...
        html = buildResultPage(
          '✅ تم تسجيل الحل بنجاح',
          'تم إغلاق البلاغ <strong>' + ticketId + '</strong> وإرسال إشعار لصاحب البلاغ.',
          '#059669',
          ticketId
        );
        return HtmlService.createHtmlOutput(html);
...
function getEmployeeByEmpId(empId) {
  ...
  for (var i = 1; i < allData.length; i++) {
    ...
    if (rEmpId === String(empId).trim() || parseInt(rEmpId) === parseInt(empId)) {
      return {
        empId:     rEmpId,
        id:        String(allData[i][idCol]||'').trim(),
        firstName: String(allData[i][fnCol]||'').trim(),
        lastName:  String(allData[i][lnCol]||'').trim(),
        role:      String(allData[i][roleCol]||'user').trim().toLowerCase(),
        email:     emailCol>=0?String(allData[i][emailCol]||'').trim():''
      };
    }
  }
  return null;
...
function notifyTeamAboutClaim(ticketId, claimant, ticketRow) {
  try {
    ...
    uD.rows.forEach(function(r) {
      ...
      if (empId === claimant.empId) return; // لا ترسل للي استلم
      ...
      var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body dir="rtl" style="font-family:Arial;padding:20px;background:#f8fafc">' +
        ...
        '<h3 style="margin:0;color:#fff;font-size:15px">📋 تم استلام بلاغ</h3>' +
        '</div>' +
        ...
        '</div></body></html>';
      try {
        MailApp.sendEmail({ to: email, subject: '📋 تم استلام البلاغ ['+ticketId+'] من قبل '+claimant.firstName+' '+claimant.lastName, htmlBody: html });
      } catch(e2) {}
    });
...
function notifyAssignee(ticketId, assignee, ticketRow) {
  ...
  try {
    var problemType   = String(ticketRow[COL_TICKETS.problemType]||'');
    var requesterName = String(ticketRow[COL_TICKETS.requesterName]||'');
    var requesterDept = String(ticketRow[COL_TICKETS.requesterDept]||'');
    var description   = String(ticketRow[COL_TICKETS.description]||'');
    var priority      = String(ticketRow[COL_TICKETS.priority]||'');
    ...
    var html = '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body dir="rtl" style="margin:0;padding:0;background:#f8fafc;font-family:Arial">' +
      ...
      '<p style="margin:6px 0 0;color:rgba(255,255,255,.7);font-size:13px">' + ticketId + '</p>' +
      '</div>' +
      '<div style="padding:24px 28px">' +
      '<p style="font-size:14px">عزيزي <strong>' + assignee.firstName + ' ' + assignee.lastName + '</strong>،</p>' +
      '<p style="font-size:13px;color:#64748b;margin-bottom:16px">تم تعيينك للبلاغ التالي:</p>' +
      ...
      '</div>' +
      '<p style="font-size:11px;color:#94a3b8;margin-top:12px">يمكنك الاستلام مباشرة من هذا الإيميل دون الحاجة لفتح النظام</p>' +
      '</div>' +
...
function buildResultPage(title, message, color, ticketId) {
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:Arial,sans-serif;background:#f8fafc;display:flex;align-items:center;justify-content:center;min-height:100vh;padding:20px}div.card{background:#fff;border-radius:16px;padding:36px 32px;max-width:420px;width:100%;text-align:center;box-shadow:0 8px 32px rgba(0,0,0,.12)}div.icon{width:70px;height:70px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:32px;margin:0 auto 20px}h2{font-size:20px;margin-bottom:12px;color:#0f172a}p{font-size:14px;color:#64748b;line-height:1.7;margin-bottom:24px}a.btn{display:inline-block;padding:12px 28px;border-radius:10px;text-decoration:none;font-size:14px;font-weight:bold;color:#fff;margin:4px}</style></head>' +
    '<body><div class="card">' +
    '<div class="icon" style="background:' + color + '20;color:' + color + '">' + (title.includes('✅')?'✅':title.includes('⚠️')?'⚠️':'❌') + '</div>' +
    '<h2 dir="rtl">' + title + '</h2>' +
    '<p dir="rtl">' + message + '</p>' +
...
function doPost_proxy(req) {
  try {
    ...
    Logger.log('doPost_proxy error: ' + e.toString());
    return fail('خطأ داخلي: ' + e.message);
  }
}
...
function getSheet(name) {
  ...
  if (!sh) {
    var allowCreate = ['InternetUsers','السجل'];
    if (allowCreate.indexOf(name) >= 0) { sh = SS.insertSheet(name); initSheet(sh, name); }
    else throw new Error('شيت "'+name+'" غير موجود');
  }
  return sh;
}
function sheetData(name) {
  ...
  if (name === SH_USERS) {
    var rm = {};
    Object.keys(COL_USERS).forEach(function(e){ rm[COL_USERS[e]]=e; });
    return { headers: vals[0].map(function(h){ return rm[h]||h; }), rows: vals.slice(1) };
  }
...
function appendRow(sheetName, obj, headers) {
  ...
  if (sheetName === SH_USERS) {
    ...
    row = rh.map(function(h){ var k=Object.keys(COL_USERS).find(function(x){return COL_USERS[x]===h;}); return (k&&obj[k]!==undefined)?obj[k]:(obj[h]!==undefined?obj[h]:''); });
  } else { row = headers.map(function(h){ return obj[h]!==undefined?obj[h]:''; }); }
  sh.appendRow(row);
}
function updateRow(sheetName, rowIndex, obj, headers) {
  var sh=getSheet(sheetName), row=headers.map(function(h){return obj[h]!==undefined?obj[h]:'';});
  sh.getRange(rowIndex+2,1,1,row.length).setValues([row]);
}
function getUsersColIdx(sh, engKey) {
  var headers = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  var ar = COL_USERS[engKey]; if(ar){var i=headers.indexOf(ar);if(i>=0)return i;}
  return headers.indexOf(engKey);
}
function arrToObj(arr, headers) {
  var obj={}; headers.forEach(function(h,i){obj[h]=arr[i]!==undefined?arr[i]:'';});return obj;
}
function initSheet(sh, name) {
  var h = { InternetUsers:['fullName','internetUser','dept'], السجل:['id','type','message','createdAt'] };
...
function buildUserObj(row,headers){function g(c){return row[headers.indexOf(c)]!==undefined?row[headers.indexOf(c)]:'';} return {id:String(g('id')),empId:String(g('empId')),firstName:String(g('firstName')),lastName:String(g('lastName')),dept:String(g('dept')),role:String(g('role')),phone:String(g('phone')||''),isAdmin:g('role')==='admin'};}
...
function updateLastLogin(userId){
  ...
  if(idCol<0)idCol=hi.indexOf('id');if(llCol<0)llCol=hi.indexOf('lastLogin');
...
function handleTicketHelp(data,user){
  var sh=getSheet(SH_TICKETS),allData=sh.getDataRange().getValues();
  for(var i=0;i<allData.length;i++){if(String(allData[i][COL_TICKETS.id]).trim()===data.ticketId){return ok({message:'تم إرسال طلب المساعدة'});}}
  return fail('البلاغ غير موجود');
}
function handleTicketCancelHelp(data,user){return ok({message:'تم إلغاء طلب المساعدة'});}
function handleTicketAttach(data,user){
  try{
    var folders=DriveApp.getFoldersByName('HelpDesk_Attachments'),folder;
    if(folders.hasNext())folder=folders.next();else folder=DriveApp.createFolder('HelpDesk_Attachments');
    var bytes=Utilities.base64Decode(data.base64),blob=Utilities.newBlob(bytes,data.mimeType||'image/jpeg',data.fileName||'attachment'),file=folder.createFile(blob);
    file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
    return ok({message:'تم رفع الملف',url:'https://drive.google.com/file/d/'+file.getId()+'/view'});
  }catch(e){return fail('خطأ في رفع الملف: '+e.message);}
}
...
function handleUserStats(user){
  ...
  allData.forEach(function(r){
    ...
    if(reqId!==user.empId&&reqId!==user.id)return;
    stats.total++;var st=String(r[COL_TICKETS.status]||'');
    if(st==='جديدة')stats.open++;if(st==='معينة')stats.assigned++;if(st==='قيد المعالجة')stats.inProgress++;if(st==='تم حل البلاغ')stats.closed++;
    myT.push(r);
  });
...
function ensureDeviceSheet() {
  var sh = SS.getSheetByName(SH_DEVICES);
  if (!sh) {
    sh = SS.insertSheet(SH_DEVICES);
    sh.appendRow(DEVICE_HEADERS);
    return sh;
  }
  // تحقق إذا فيه headers
  var firstRow = sh.getRange(1,1,1,sh.getLastColumn()).getValues()[0];
  var hasHeaders = firstRow.some(function(h){ return String(h).trim() === 'devId'; });
  if (!hasHeaders) {
    // أضف headers في الصف الأول وانقل البيانات للأسفل
    sh.insertRowBefore(1);
    sh.getRange(1,1,1,DEVICE_HEADERS.length).setValues([DEVICE_HEADERS]);
  }
  return sh;
}

function handleDeviceCheckin(data,user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var devId=genId('DEV'),sh=ensureDeviceSheet();
  var hi=sh.getRange(1,1,1,Math.max(sh.getLastColumn(),DEVICE_HEADERS.length)).getValues()[0];
  var row=new Array(Math.max(hi.length,DEVICE_HEADERS.length)).fill('');
  function setD(k,v){var i=hi.indexOf(k);if(i>=0)row[i]=v;}
  setD('devId',devId);setD('ticketId',data.ticketId||'');setD('deviceType',data.deviceType||'');
  setD('deviceDesc',data.deviceDesc||'');setD('ownerName',data.ownerName||'');setD('ownerDept',data.ownerDept||'');
  setD('status','عند IT');setD('checkinBy',user.firstName+' '+user.lastName);
  setD('checkinAt',now());setD('finalNotes',data.notes||'');
  sh.appendRow(row);return ok({message:'تم تسجيل استلام الجهاز',devId:devId});
}
function handleDeviceSendToTech(data,user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var uD=sheetData(SH_USERS),uH=uD.headers,tn='';
  for(var i=0;i<uD.rows.length;i++){if(String(uD.rows[i][uH.indexOf('id')])===data.techId){tn=String(uD.rows[i][uH.indexOf('firstName')])+' '+String(uD.rows[i][uH.indexOf('lastName')]);break;}}
  return updateDeviceField(data.devId,{status:'عند الفني',techId:data.techId||'',techName:tn,sentToTechAt:now(),finalNotes:data.notes||''});
}
function handleDeviceTechRepair(data,user){
  if(ROLES_TECH.indexOf(user.role)<0)return fail('غير مصرح');
  var labels={fixed:'تم الإصلاح الكامل',partial:'إصلاح جزئي',failed:'تعذّر الإصلاح',needs_parts:'يحتاج قطع غيار'};
  return updateDeviceField(data.devId,{status:data.readyForPickup?'أُنجز — بانتظار IT':'عند الفني',techResult:data.result||'fixed',techResultLabel:labels[data.result]||data.result,workDone:data.workDone||'',failReason:data.failReason||'',techDoneAt:now()});
}
...
function handleDeviceFinalDeliver(data,user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var res=updateDeviceField(data.devId,{status:'مُسلَّم',deliveredAt:now(),finalCondition:data.condition||'',finalNotes:data.notes||''});
  if(!res.success)return res;
  var sh=SS.getSheetByName(SH_DEVICES),allD=sh?sh.getDataRange().getValues():[];
  var hi=allD[0]||[],tid='';
...
function handleDevicesList(data,user){
  var sh=ensureDeviceSheet();
  var allData=sh.getDataRange().getValues();if(allData.length<2)return ok({devices:[]});
  var hi=allData[0],statusF=data.status||'all',devices=[];
  for(var i=allData.length-1;i>=1;i--){
    var r=allData[i];function gd(c){var ci=hi.indexOf(c);return ci>=0?String(r[ci]||'').trim():'';}
    var devId=gd('devId');if(!devId)continue;
    var status=gd('status');if(statusF!=='all'&&status!==statusF)continue;
    if(user.role==='tech'&&gd('techId')!==user.id&&gd('techId')!==user.empId)continue;
    devices.push({devId:devId,ticketId:gd('ticketId'),deviceType:gd('deviceType'),deviceDesc:gd('deviceDesc'),ownerName:gd('ownerName'),ownerDept:gd('ownerDept'),status:status,checkinBy:gd('checkinBy'),checkinAt:gd('checkinAt'),techId:gd('techId'),techName:gd('techName'),sentToTechAt:gd('sentToTechAt'),techResult:gd('techResult'),techResultLabel:gd('techResultLabel'),workDone:gd('workDone'),failReason:gd('failReason'),techDoneAt:gd('techDoneAt'),itReceivedAt:gd('itReceivedAt'),itNotes:gd('itNotes'),deliveredAt:gd('deliveredAt'),finalCondition:gd('finalCondition')});
  }
  return ok({devices:devices});
}
function updateDeviceField(devId,updates){
  var sh=ensureDeviceSheet();
  var d=sh.getDataRange().getValues();if(d.length<1)return fail('الجهاز غير موجود');
  var hi=d[0],devIdCol=hi.indexOf('devId');if(devIdCol<0)devIdCol=0;
  for(var i=1;i<d.length;i++){if(String(d[i][devIdCol]).trim()===devId){Object.keys(updates).forEach(function(k){var col=hi.indexOf(k);if(col>=0)sh.getRange(i+1,col+1).setValue(updates[k]);});return ok({message:'تم تحديث الجهاز'});}}
  return fail('الجهاز غير موجود: '+devId);
}
...
function handleUsersList(user){
  ...
  var sh=getSheet(SH_USERS),allData=sh.getDataRange().getValues(),rh=allData[0];
  function ci(k){return rh.indexOf(COL_USERS[k]);}
  var users=[];
  for(var i=1;i<allData.length;i++){
    var r=allData[i];function g(k){var c=ci(k);return c>=0?String(r[c]||'').trim():'';}
    var empId=g('empId');if(!empId)continue;
    var av=ci('active')>=0?r[ci('active')]:true;
...
function handleUsersAdd(data,user){
  ...
  var d=sheetData(SH_USERS),hi=d.headers;
  for(var i=0;i<d.rows.length;i++){if(String(d.rows[i][hi.indexOf('empId')]).toUpperCase()===empId)return fail('رقم البصمة موجود مسبقاً');}
...
function handleUsersToggle(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');
  var sh=getSheet(SH_USERS),d=sh.getDataRange().getValues(),idCol=getUsersColIdx(sh,'id'),acCol=getUsersColIdx(sh,'active');
  for(var i=1;i<d.length;i++){if(String(d[i][idCol])===data.id){var cur=d[i][acCol]===true||String(d[i][acCol]).toLowerCase()==='true';sh.getRange(i+1,acCol+1).setValue(!cur);return ok({message:(!cur?'تم تفعيل':'تم تعطيل')+' الحساب'});}}
  return fail('الموظف غير موجود');
}
function handleUsersDelete(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');
  var sh=getSheet(SH_USERS),d=sh.getDataRange().getValues(),idCol=getUsersColIdx(sh,'id');
  for(var i=1;i<d.length;i++){if(String(d[i][idCol])===data.id){sh.deleteRow(i+1);return ok({message:'تم حذف الموظف'});}}
  return fail('الموظف غير موجود');
}
function handleUsersResetPw(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');
  var sh=getSheet(SH_USERS),d=sh.getDataRange().getValues(),idCol=getUsersColIdx(sh,'id'),pwCol=getUsersColIdx(sh,'pwHash');
  for(var i=1;i<d.length;i++){if(String(d[i][idCol])===data.id){sh.getRange(i+1,pwCol+1).setValue(data.pwHash);return ok({message:'تم تغيير الرمز السري'});}}
  return fail('الموظف غير موجود');
}
function handleUserMyInfo(user){
  var result=getInternetUserForEmployee(user.firstName,user.lastName,user.dept);
  var internetUser=result?result.internetUser:'';
  var tSh=getSheet(SH_TICKETS),tData=tSh.getDataRange().getValues();
  var ticketCount=tData.filter(function(r){var tid=String(r[COL_TICKETS.id]||'').trim();if(!tid||tid.indexOf('TKT')<0)return false;var rid=String(r[COL_TICKETS.requesterId]||'').trim();return rid===user.empId||rid===user.id;}).length;
  return ok({internetUser:internetUser||null,ticketCount:ticketCount,phone:user.phone||''});
}

// ══════ INTERNET USERS + SMART MATCHING ══════
function smartNameMatch(a,b){
  if(!a||!b)return 0;
  function norm(n){return n.trim().replace(/\s+/g,' ').replace(/[أإآ]/g,'ا').replace(/[ةه]/g,'ه').replace(/[يى]/g,'ي').toLowerCase();}
  var na=norm(a),nb=norm(b);if(na===nb)return 1.0;
  var ap=na.split(' ').filter(function(p){return p.length>0;}),bp=nb.split(' ').filter(function(p){return p.length>0;});
  var shorter=ap.length<=bp.length?ap:bp,longer=ap.length<=bp.length?bp:ap;
  var li=-1,mc=0,af=true;
  for(var k=0;k<shorter.length;k++){var f=false;for(var j=li+1;j<longer.length;j++){if(longer[j]===shorter[k]||longer[j].indexOf(shorter[k])===0||shorter[k].indexOf(longer[j])===0){li=j;mc++;f=true;break;}}if(!f){af=false;break;}}
  if(af&&mc===shorter.length)return 0.85+0.15*mc/longer.length;
  var aSet={},bSet={};ap.forEach(function(p){aSet[p]=true;});bp.forEach(function(p){bSet[p]=true;});
  var inter=Object.keys(aSet).filter(function(k){return bSet[k];}).length,us=Object.keys(Object.assign({},aSet,bSet)).length;
  return us>0?inter/us:0;
}
function findInternetUserByName(fullName){
  try{
    var sh=SS.getSheetByName(SH_IUSERS);if(!sh)return{status:'not_found'};
    var allData=sh.getDataRange().getValues();if(allData.length<2)return{status:'not_found'};
    var candidates=[],THRESHOLD=0.72;
    for(var i=1;i<allData.length;i++){
      var sName=String(allData[i][0]||'').trim(),sUser=String(allData[i][1]||'').trim(),sDept=String(allData[i][2]||'').trim();
      if(!sName||!sUser)continue;
      var score=smartNameMatch(fullName,sName);if(score>=THRESHOLD)candidates.push({sheetName:sName,internetUser:sUser,dept:sDept,score:score});
    }
    if(!candidates.length)return{status:'not_found'};
    candidates.sort(function(a,b){return b.score-a.score;});
    var top=candidates[0],runner=candidates[1];
    if(runner&&(top.score-runner.score)<0.05&&top.score<0.95)return{status:'ambiguous',candidates:candidates.slice(0,3).map(function(c){return{name:c.sheetName,user:c.internetUser,score:Math.round(c.score*100)};})};
    return{status:'found',internetUser:top.internetUser,matchedName:top.sheetName,dept:top.dept,confidence:Math.round(top.score*100)};
  }catch(e){return{status:'not_found'};}
}
function getInternetUserForEmployee(firstName,lastName,dept){
  var fullName=(firstName+' '+lastName).trim(),sh=SS.getSheetByName(SH_IUSERS);if(!sh)return null;
  var allData=sh.getDataRange().getValues().slice(1);
  for(var i=0;i<allData.length;i++){
    var sName=String(allData[i][0]||'').trim(),sUser=String(allData[i][1]||'').trim(),sDept=String(allData[i][2]||'').trim();
    if(!sName||!sUser)continue;
    if(sName===fullName)return{internetUser:sUser,confidence:100,matchedName:sName};
    var ns=smartNameMatch(fullName,sName);
    if(ns>=0.80){var dm=dept&&sDept&&(dept.includes(sDept)||sDept.includes(dept)),fs=dm?Math.min(ns+0.10,1.0):ns;if(fs>=0.80)return{internetUser:sUser,confidence:Math.round(fs*100),matchedName:sName};}
  }
  return null;
}
function saveInternetUser(empId,fullName,internetUser,dept,updatedBy){
  var sh=SS.getSheetByName(SH_IUSERS);if(!sh){sh=SS.insertSheet(SH_IUSERS);sh.appendRow(['fullName','internetUser','dept']);}
  var allData=sh.getDataRange().getValues();
  for(var i=1;i<allData.length;i++){if(smartNameMatch(fullName,String(allData[i][0]||'').trim())>=0.9){sh.getRange(i+1,2).setValue(internetUser);return;}}
  sh.appendRow([fullName,internetUser,dept||'']);
}
function handleInternetUsersList(user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var iSh=SS.getSheetByName(SH_IUSERS),iData=iSh?iSh.getDataRange().getValues().slice(1):[];
  var nameMap={};iData.forEach(function(r){var n=String(r[0]||'').trim(),u=String(r[1]||'').trim(),d=String(r[2]||'').trim();if(n)nameMap[n]={internetUser:u,dept:d};});
  var uD=sheetData(SH_USERS),uH=uD.headers;
  var users=uD.rows.map(function(r){
    function g(c){return String(r[uH.indexOf(c)]!==undefined?r[uH.indexOf(c)]:'');}
    var empId=g('empId').toUpperCase(),fullName=(g('firstName')+' '+g('lastName')).trim();
    var info=nameMap[fullName]||{};
    if(!info.internetUser){var m=findInternetUserByName(fullName);if(m&&m.status==='found')info={internetUser:m.internetUser,dept:m.dept||''};}
    return{empId:empId,name:fullName,dept:g('dept'),internetUser:info.internetUser||'',updatedAt:''};
  }).filter(function(u){return u.empId;});
  return ok({users:users});
}
function handleInternetUsersSet(data,user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var empId=(data.empId||'').toString().trim().toUpperCase(),iUser=(data.internetUser||'').toString().trim().toLowerCase();
  if(!empId||!iUser)return fail('البيانات ناقصة');
  var uD=sheetData(SH_USERS),uH=uD.headers,fullName='',dept='';
  for(var i=0;i<uD.rows.length;i++){if(String(uD.rows[i][uH.indexOf('empId')]).toUpperCase()===empId){fullName=(String(uD.rows[i][uH.indexOf('firstName')])+' '+String(uD.rows[i][uH.indexOf('lastName')])).trim();dept=String(uD.rows[i][uH.indexOf('dept')]||'');break;}}
  if(!fullName)return fail('رقم البصمة '+empId+' غير موجود');
  saveInternetUser(empId,fullName,iUser,dept,user.firstName+' '+user.lastName);
  return ok({message:'تم حفظ يوزر الإنترنت: '+iUser+' للموظف '+fullName});
}
function handleInternetUsersSearch(data,user){
  if(ROLES_IT.indexOf(user.role)<0)return fail('غير مصرح');
  var empId=(data.empId||'').toString().trim().toUpperCase();if(!empId)return fail('ادخل رقم البصمة');
  var uD=sheetData(SH_USERS),uH=uD.headers,fullName='',dept='',found=false;
  for(var i=0;i<uD.rows.length;i++){if(String(uD.rows[i][uH.indexOf('empId')]).toUpperCase()===empId){fullName=(String(uD.rows[i][uH.indexOf('firstName')])+' '+String(uD.rows[i][uH.indexOf('lastName')])).trim();dept=String(uD.rows[i][uH.indexOf('dept')]||'');found=true;break;}}
  if(!found)return fail('رقم البصمة '+empId+' غير موجود بالنظام');
  var iSh=SS.getSheetByName(SH_IUSERS),internetUser='',matchedName='',confidence=0;
  if(iSh){
    var iData=iSh.getDataRange().getValues().slice(1);
    for(var j=0;j<iData.length;j++){var sN=String(iData[j][0]||'').trim(),sU=String(iData[j][1]||'').trim();if(sN===fullName&&sU){internetUser=sU;matchedName=sN;confidence=100;break;}}
    if(!internetUser){var m=findInternetUserByName(fullName);if(m&&m.status==='found'){internetUser=m.internetUser;matchedName=m.matchedName;confidence=m.confidence;}}
  }
  return ok({empId:empId,fullName:fullName,dept:dept,internetUser:internetUser||null,matchedName:matchedName||null,confidence:confidence});
}

// ══════ GUIDELINES ══════
function handleGuidelinesList(){
  var sh=SS.getSheetByName(SH_GUIDELINES);if(!sh)return ok({guidelines:[]});
  var allData=sh.getDataRange().getValues();if(allData.length<1)return ok({guidelines:[]});
  var headers=allData[0].map(function(h){return String(h).trim();}),list=[];
  for(var i=1;i<allData.length;i++){
    var r=allData[i];
    function gf(keys){for(var k=0;k<keys.length;k++){var idx=headers.indexOf(keys[k]);if(idx>=0&&r[idx]!==undefined&&String(r[idx]).trim())return String(r[idx]).trim();}return '';}
    var id=gf(['id','رقم','ID'])||String(i),title=gf(['title','عنوان','العنوان','التوجيه']),content=gf(['content','محتوى','المحتوى','التفاصيل','الوصف']);
    if(!title&&!content)continue;
    list.push({id:id,title:title,content:content,icon:gf(['icon','أيقونة','ايقونة'])||'⚠️',priority:gf(['priority','أهمية','الأهمية','الأولوية'])||'مهم',createdAt:gf(['createdAt','تاريخ','التاريخ'])});
  }
  return ok({guidelines:list});
}
function handleGuidelinesAdd(data,user){
  if(!data.title||!data.content)return fail('العنوان والمحتوى مطلوبان');
  if(['it','it_manager','admin'].indexOf(user.role)<0)return fail('غير مصرح');
  var sh=SS.getSheetByName(SH_GUIDELINES);if(!sh)return fail('شيت التوجيهات غير موجود');
  var headers=sh.getDataRange().getValues()[0].map(function(h){return String(h).trim();}),row=new Array(headers.length).fill('');
  function sc(keys,val){for(var k=0;k<keys.length;k++){var i=headers.indexOf(keys[k]);if(i>=0){row[i]=val;return;}}}
  sc(['id','رقم'],genId('GD'));sc(['title','عنوان','العنوان','التوجيه'],data.title);sc(['content','محتوى','المحتوى','التفاصيل'],data.content);sc(['icon','أيقونة'],data.icon||'⚠️');sc(['priority','أهمية','الأهمية'],data.priority||'مهم');sc(['createdAt','تاريخ','التاريخ'],now());
  sh.appendRow(row);return ok({message:'تمت إضافة التوجيه'});
}
function handleGuidelinesUpdate(data,user){
  if(['it','it_manager','admin'].indexOf(user.role)<0)return fail('غير مصرح');
  var sh=SS.getSheetByName(SH_GUIDELINES);if(!sh)return fail('غير موجود');
  var d=sh.getDataRange().getValues(),hi=d[0];
  function fi(keys){for(var k=0;k<keys.length;k++){var i=hi.indexOf(keys[k]);if(i>=0)return i;}return -1;}
  var idCol=fi(['id','رقم','ID']);
  for(var i=1;i<d.length;i++){
    if(String(d[i][idCol])===data.id){
      var tc=fi(['title','عنوان','العنوان','التوجيه']),cc=fi(['content','محتوى','المحتوى']),ic=fi(['icon','أيقونة']),pc=fi(['priority','أهمية']);
      if(data.title&&tc>=0)sh.getRange(i+1,tc+1).setValue(data.title);
      if(data.content&&cc>=0)sh.getRange(i+1,cc+1).setValue(data.content);
      if(data.icon&&ic>=0)sh.getRange(i+1,ic+1).setValue(data.icon);
      if(data.priority&&pc>=0)sh.getRange(i+1,pc+1).setValue(data.priority);
      return ok({message:'تم التعديل'});
    }
  }
  return fail('غير موجود');
}
function handleGuidelinesDelete(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');
  var sh=SS.getSheetByName(SH_GUIDELINES);if(!sh)return fail('غير موجود');
  var d=sh.getDataRange().getValues(),hi=d[0];
  function fi(keys){for(var k=0;k<keys.length;k++){var i=hi.indexOf(keys[k]);if(i>=0)return i;}return -1;}
  var idCol=fi(['id','رقم','ID']);
  for(var i=1;i<d.length;i++){if(String(d[i][idCol])===data.id){sh.deleteRow(i+1);return ok({message:'تم الحذف'});}}
  return fail('غير موجود');
}

// ══════ MANUAL ══════
function handleManualList(data){
  var sh=SS.getSheetByName(SH_MANUAL);if(!sh)return ok({entries:[]});
  var allData=sh.getDataRange().getValues();if(allData.length<1)return ok({entries:[]});
  var headers=allData[0].map(function(h){return String(h).trim();}),device=(data.device||'').toLowerCase(),entries=[];
  for(var i=1;i<allData.length;i++){
    var r=allData[i];
    function gm(keys){for(var k=0;k<keys.length;k++){var idx=headers.indexOf(keys[k]);if(idx>=0&&r[idx]!==undefined)return String(r[idx]||'').trim();}return '';}
    var id=gm(['id','رقم'])||String(i),dev=gm(['device','جهاز','نوع_الجهاز','نوع الجهاز']),title=gm(['title','عنوان','العنوان']),sr=gm(['steps','خطوات','الخطوات']);
    var steps=[];try{steps=JSON.parse(sr);}catch(e){steps=sr?sr.split('\n').filter(Boolean):[];}
    if(!title&&!steps.length)continue;if(device&&dev.toLowerCase()!==device)continue;
    entries.push({id:id,device:dev,problemType:gm(['problemType','مشكلة','نوع_المشكلة']),title:title,description:gm(['description','وصف','الوصف']),icon:gm(['icon','أيقونة'])||'💻',steps:steps});
  }
  return ok({entries:entries});
}
function handleManualAdd(data,user){
  if(['it','it_manager','admin'].indexOf(user.role)<0)return fail('غير مصرح');
  if(!data.title||!data.device||!data.steps||!data.steps.length)return fail('البيانات ناقصة');
  var sh=SS.getSheetByName(SH_MANUAL);if(!sh)return fail('شيت كتيب التعليمات غير موجود');
  var headers=sh.getDataRange().getValues()[0].map(function(h){return String(h).trim();}),row=new Array(headers.length).fill('');
  function sc(keys,val){for(var k=0;k<keys.length;k++){var i=headers.indexOf(keys[k]);if(i>=0){row[i]=val;return;}}}
  sc(['id','رقم'],genId('MN'));sc(['device','جهاز','نوع_الجهاز'],data.device);sc(['title','عنوان','العنوان'],data.title);sc(['steps','خطوات','الخطوات'],JSON.stringify(data.steps));sc(['problemType','مشكلة'],data.problemType||'');sc(['description','وصف'],data.description||'');sc(['icon','أيقونة'],data.icon||'💻');sc(['createdAt','تاريخ'],now());
  sh.appendRow(row);return ok({message:'تمت إضافة الدليل'});
}
function handleManualUpdate(data,user){
  if(['it','it_manager','admin'].indexOf(user.role)<0)return fail('غير مصرح');
  var sh=SS.getSheetByName(SH_MANUAL);if(!sh)return fail('غير موجود');
  var d=sh.getDataRange().getValues(),hi=d[0];
  function fi(keys){for(var k=0;k<keys.length;k++){var i=hi.indexOf(keys[k]);if(i>=0)return i;}return -1;}
  var idCol=fi(['id','رقم']);
  for(var i=1;i<d.length;i++){
    if(String(d[i][idCol])===data.id){
      function sv(keys,val){if(val===undefined||val===null)return;var c=fi(keys);if(c>=0)sh.getRange(i+1,c+1).setValue(val);}
      sv(['title','عنوان'],data.title);sv(['device','جهاز'],data.device);sv(['problemType','مشكلة'],data.problemType);sv(['description','وصف'],data.description);sv(['icon','أيقونة'],data.icon);
      if(data.steps){var sc2=fi(['steps','خطوات']);if(sc2>=0)sh.getRange(i+1,sc2+1).setValue(JSON.stringify(data.steps));}
      return ok({message:'تم التعديل'});
    }
  }
  return fail('غير موجود');
}
function handleManualDelete(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');
  var sh=SS.getSheetByName(SH_MANUAL);if(!sh)return fail('غير موجود');
  var d=sh.getDataRange().getValues(),hi=d[0];
  function fi(keys){for(var k=0;k<keys.length;k++){var i=hi.indexOf(keys[k]);if(i>=0)return i;}return -1;}
...
function handleDeptsList(){
  var sh=getSheet(SH_DEPTS),allData=sh.getDataRange().getValues();if(!allData.length)return ok({depts:[]});
  var headers=allData[0],nameCol=-1;
  for(var h=0;h<headers.length;h++){var hs=String(headers[h]).trim();if(hs.includes('قسم')||hs.includes('name')||hs==='اسم'){nameCol=h;break;}}
  if(nameCol<0)nameCol=0;
  var depts=[];for(var i=1;i<allData.length;i++){var val=String(allData[i][nameCol]||'').trim();if(val)depts.push(val);}
  return ok({depts:depts});
}
function handleDeptsAdd(data,user){
  if(ROLES_ADMIN.indexOf(user.role)<0)return fail('غير مصرح');if(!data.name)return fail('اسم القسم مطلوب');
  var sh=getSheet(SH_DEPTS),allData=sh.getDataRange().getValues(),headers=allData[0],nameCol=0;
  for(var h=0;h<headers.length;h++){if(String(headers[h]).includes('قسم')||String(headers[h]).toLowerCase().includes('name')){nameCol=h;break;}}
...
function handleDeptsDelete(data,user){
  ...
  var sh=getSheet(SH_DEPTS),allData=sh.getDataRange().getValues(),headers=allData[0],nameCol=0;
  for(var h=0;h<headers.length;h++){if(String(headers[h]).includes('قسم')||String(headers[h]).toLowerCase().includes('name')){nameCol=h;break;}}
  for(var i=1;i<allData.length;i++){if(String(allData[i][nameCol]).trim()===data.name.trim()){sh.deleteRow(i+1);return ok({message:'تم حذف القسم'});}}
...
function handleNotificationsCheck(data,user){
  ...
  allData.forEach(function(r){
    var tid=String(r[COL_TICKETS.id]||'').trim();if(!tid||tid.indexOf('TKT')<0)return;
    if(String(r[COL_TICKETS.status]||'')!=='جديدة')return;
    var created=String(r[COL_TICKETS.createdAt]||'');
    try{var p=created.split(/[\/\s:]/),cd=new Date(p[2],p[1]-1,p[0],p[3]||0,p[4]||0);if(cd>since)newTickets.push({id:tid,problemType:String(r[COL_TICKETS.problemType]||''),requesterName:String(r[COL_TICKETS.requesterName]||''),requesterDept:String(r[COL_TICKETS.requesterDept]||''),priority:String(r[COL_TICKETS.priority]||'')});}catch(e2){}
  });
...
function buildTicketEmail(ticket, claimUrl, solveUrl){
  claimUrl = claimUrl || SYSTEM_URL;
  solveUrl = solveUrl || SYSTEM_URL;
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body dir="rtl" style="margin:0;padding:0;background:#f8fafc;font-family:Arial,sans-serif">' +
    ...
    '<tr><td style="padding:10px 14px;background:#f8fafc;font-weight:bold;font-size:13px;border-radius:6px 0 0 6px;width:35%">المُبلِّغ</td><td style="padding:10px 14px;font-size:13px">' + ticket.requesterName + '</td></tr>' +
    ...
    '<tr><td style="padding:10px 14px;background:#f8fafc;font-weight:bold;font-size:13px">الأولوية</td><td style="padding:10px 14px"><span style="background:' + priorityColor + '20;color:' + priorityColor + ';padding:3px 10px;border-radius:12px;font-size:12px;font-weight:bold">' + ticket.priority + '</span></td></tr>' +
...
// ══════ AI CHAT — مع التعلم التلقائي من حلول IT ويوزر الإنترنت ══════
var AI_MODEL = 'claude-sonnet-4-20250514';

function getITKnowledge(){
  // يجمع الحلول المسجلة من البلاغات المحلولة لتغذية الذكاء الاصطناعي
  try{
    var sh=getSheet(SH_TICKETS),allData=sh.getDataRange().getValues(),byType={};
    allData.forEach(function(r){
      var tid=String(r[COL_TICKETS.id]||'').trim();if(!tid||tid.indexOf('TKT')<0)return;
      if(String(r[COL_TICKETS.status]||'')!=='تم حل البلاغ')return;
      // نستخدم الوصف كحل لأن شيتك ما عنده عمود solution منفصل
      var desc=String(r[COL_TICKETS.description]||'').trim();
      var prob=String(r[COL_TICKETS.problemType]||'').trim();
      var dev=String(r[COL_TICKETS.deviceId]||'').trim();
      if(!prob||!desc)return;
      var key=prob+(dev?'|'+dev:'');
      if(!byType[key])byType[key]={count:0,desc:[],type:prob,device:dev};
      byType[key].count++;if(byType[key].desc.length<2)byType[key].desc.push(desc.slice(0,100));
    });
    var lines=Object.values(byType).sort(function(a,b){return b.count-a.count;}).slice(0,10).map(function(g){return '['+g.type+(g.device?' | '+g.device:'')+'] حدث '+g.count+' مرة';});
...
function handleAIChat(data,user){
  ...
  // جلب يوزر الإنترنت للموظف
  var internetUser='';
  try{var iuResult=getInternetUserForEmployee(user.firstName,user.lastName,user.dept);if(iuResult)internetUser=iuResult.internetUser;}catch(e){}

  // بناء الـ system prompt مع المعرفة المكتسبة
  var knowledge='';
  try{knowledge=getITKnowledge();}catch(e){}
  ...
  try{
    ...
    if(body.error)return fail('خطأ من الذكاء الاصطناعي: '+body.error.message);
    var reply=body.content&&body.content[0]?body.content[0].text:'';
    var submitTicket=reply.indexOf('[TICKET]')>=0;
    reply=reply.replace('[TICKET]','').trim();
    return ok({reply:reply,submitTicket:submitTicket});
  }catch(e){return fail('تعذر الاتصال بالذكاء الاصطناعي: '+e.message);}
}

function buildAISystemPrompt(user,internetUser,knowledge){
  var iuSection=internetUser
    ?'يوزر الإنترنت للموظف الحالي: '+internetUser+' — إذا سأل عن يوزره أعطه هذا مباشرة.'
    :'الموظف الحالي ليس عنده يوزر إنترنت مسجل — إذا سأل عنه قله يتواصل مع فريق IT.';
  var knowledgeSection=knowledge
    ?'## أكثر المشاكل تكراراً وحلولها من سجل IT:\n'+knowledge
    :'## لا توجد حلول مسجلة بعد — استخدم خبرتك التقنية العامة.';
  return 'أنت مساعد تقني ذكي لشركة عراقية. اسم الموظف: '+user.firstName+' '+user.lastName+' | القسم: '+user.dept+'\n\n'+iuSection+'\n\n'+knowledgeSection+'\n\n## قواعد صارمة:\n- لا تذكر أي IP أو كلمة مرور أو معلومات شبكية داخلية\n- أجب باللهجة العراقية البسيطة\n- كل رد: تشخيص مختصر + خطوة عملية واحدة\n- بعد 5 خطوات فاشلة: اقترح بلاغ بكلمة [TICKET]';
}
...
// ══════════════════════════════
//  stats.tech — تقارير الفني
// ══════════════════════════════
function handleTechStats(user) {
  if (user.role !== 'tech') return fail('غير مصرح');
  var sh = SS.getSheetByName(SH_DEVICES);
  if (!sh) return ok({ stats: { total:0, fixed:0, pending:0, delivered:0 }, recent: [] });
  var allData = sh.getDataRange().getValues();
  if (allData.length < 2) return ok({ stats: { total:0, fixed:0, pending:0, delivered:0 }, recent: [] });
  var hi = allData[0];
  function ci(k) { return hi.indexOf(k); }
  var stats = { total:0, fixed:0, pending:0, delivered:0 };
  var myDevices = [];
  for (var i = 1; i < allData.length; i++) {
    var r = allData[i];
    function gv(k) { var c=ci(k); return c>=0?String(r[c]||'').trim():''; }
    var techId = gv('techId');
    if (techId !== user.id && techId !== user.empId) continue;
    stats.total++;
    var status = gv('status');
    if (status === 'مُسلَّم') stats.delivered++;
    else if (status === 'أُنجز — بانتظار IT' || status === 'جاهز للتسليم') stats.fixed++;
    else stats.pending++;
    myDevices.push({
      devId: gv('devId'), deviceType: gv('deviceType'), deviceDesc: gv('deviceDesc'),
      ownerName: gv('ownerName'), ownerDept: gv('ownerDept'), status: status,
      techResult: gv('techResult'), techResultLabel: gv('techResultLabel'),
      workDone: gv('workDone'), techDoneAt: gv('techDoneAt'), checkinAt: gv('checkinAt')
    });
  }
  // آخر 10 أجهزة
  var recent = myDevices.slice(-10).reverse();
  return ok({ stats: stats, recent: recent });
}
