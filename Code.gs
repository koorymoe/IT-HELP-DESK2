...
function handleGuidelinesList(){
  ...
  var idCol=headers.indexOf('id')>=0?headers.indexOf('id'):(headers.indexOf('رقم')>=0?headers.indexOf('رقم'):(headers.indexOf('ID')>=0?headers.indexOf('ID'):-1));
  var titleCol=headers.indexOf('title')>=0?headers.indexOf('title'):(headers.indexOf('عنوان')>=0?headers.indexOf('عنوان'):(headers.indexOf('العنوان')>=0?headers.indexOf('العنوان'):headers.indexOf('التوجيه')));
  var contentCol=headers.indexOf('content')>=0?headers.indexOf('content'):(headers.indexOf('محتوى')>=0?headers.indexOf('محتوى'):(headers.indexOf('المحتوى')>=0?headers.indexOf('المحتوى'):(headers.indexOf('التفاصيل')>=0?headers.indexOf('التفاصيل'):headers.indexOf('الوصف'))));
  var iconCol=headers.indexOf('icon')>=0?headers.indexOf('icon'):(headers.indexOf('أيقونة')>=0?headers.indexOf('أيقونة'):headers.indexOf('ايقونة'));
  var priCol=headers.indexOf('priority')>=0?headers.indexOf('priority'):(headers.indexOf('أهمية')>=0?headers.indexOf('أهمية'):(headers.indexOf('الأهمية')>=0?headers.indexOf('الأهمية'):headers.indexOf('الأولوية')));
  var catCol=headers.indexOf('createdAt')>=0?headers.indexOf('createdAt'):(headers.indexOf('تاريخ')>=0?headers.indexOf('تاريخ'):headers.indexOf('التاريخ'));
  function gc(row,col){return col>=0&&row[col]!==undefined?String(row[col]||'').trim():'';}
  var list=[];
  for(var i=1;i<allData.length;i++){
    var r=allData[i];
    var title=gc(r,titleCol),content=gc(r,contentCol);
    if(!title&&!content)continue;
    list.push({id:gc(r,idCol)||String(i),title:title,content:content,icon:gc(r,iconCol)||'⚠️',priority:gc(r,priCol)||'مهم',createdAt:gc(r,catCol)});
  }
...
function handleManualList(data){
  ...
  var headers=allData[0].map(function(h){return String(h).trim();});
  var device=(data&&data.device||'').toLowerCase();
  function mCol(keys){for(var k=0;k<keys.length;k++){var i=headers.indexOf(keys[k]);if(i>=0)return i;}return -1;}
  var idCol=mCol(['id','رقم']),devCol=mCol(['device','جهاز','نوع_الجهاز','نوع الجهاز']),titleCol=mCol(['title','عنوان','العنوان']),stepsCol=mCol(['steps','خطوات','الخطوات']),ptCol=mCol(['problemType','مشكلة','نوع_المشكلة']),descCol=mCol(['description','وصف','الوصف']),iconCol=mCol(['icon','أيقونة']);
  function gc(row,col){return col>=0&&row[col]!==undefined?String(row[col]||'').trim():'';}
  var entries=[];
  for(var i=1;i<allData.length;i++){
    var r=allData[i];
    var title=gc(r,titleCol),sr=gc(r,stepsCol),dev=gc(r,devCol);
    var steps=[];try{steps=JSON.parse(sr);}catch(e){steps=sr?sr.split('\n').filter(Boolean):[];}
    if(!title&&!steps.length)continue;
    if(device&&dev.toLowerCase()!==device)continue;
    entries.push({id:gc(r,idCol)||String(i),device:dev,problemType:gc(r,ptCol),title:title,description:gc(r,descCol),icon:gc(r,iconCol)||'💻',steps:steps});
  }
